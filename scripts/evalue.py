...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(32 * 64 * 11, 128),  # Cambiar 61 * 61 a 61 * 11
            nn.ReLU(),
...
model = torch.load("modelo_ladrillo.pth", map_location=device)
model.to(device)


# Cargar los pesos entrenados
model.load_state_dict(torch.load("modelo_ladrillo.pth"))
model.eval()
